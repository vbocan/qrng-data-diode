...
Public QRNG services from ANU and NIST democratize quantum randomness access but impose limitations unsuitable for research-grade applications [4][5]: ANU limits requests to 5/sec. with 1024-byte maximums and 450ms latency, while NIST provides only one pulse per minute. These services require Internet connectivity, raising privacy concerns through centralized request logging. QRNG-DD's self-hosted architecture eliminates rate limits, supports megabyte requests, achieves single-digit millisecond latencies, and ensures complete privacy.
...
QRNG-DD offers distinct advantages in cost-effectiveness, performance, and features compared to existing solutions. Open-source MIT licensing enables independent security audits, customization for specific requirements, and reproducible research, while zero software cost provides significant economic advantage over hardware data diodes typically requiring thousand of dollars investment. Organizations with existing QRNG appliances add QRNG-DD without additional hardware.

Performance demonstrates substantial advantages with sub-4-millisecond median latency representing more than one-hundred-fold improvement over ANU's 450-millisecond average, while sustained throughput depends on the connected QRNG appliance's entropy generation rate, typically achieving approximately 29 requests per second for 1-kilobyte requests from a single appliance. The gateway software itself demonstrates burst capability exceeding 400 requests per second when the buffer contains sufficient entropy. Self-hosted deployment supporting megabyte requests contrasts with public service 1024-byte limits, while MCP integration enables AI agent access to quantum randomness. Production observability through Prometheus metrics, structured logging, and Docker deployment provides operational capabilities absent from prototypes and public services.

Technology selection depends on specific requirements: hardware data diodes remain appropriate for critical infrastructure requiring absolute physical guarantees, QRNG-DD provides adequate security for research and moderate-security applications with throughput requirements matching available QRNG appliance capacity, pseudo-random generators suit high-speed applications where cryptographic unpredictability is unnecessary, and public services suffice for occasional low-volume needs. Organizations requiring higher throughput can deploy multiple QRNG appliances with multi-source aggregation to scale entropy generation capacity proportionally.
...
Cryptographic integrity verification through test suites implementing adversarial scenarios including tampered packets, replay attempts, expired timestamps, and sequence gaps correctly rejects all malicious packets with appropriate error codes. Constant-time HMAC comparison shows standard deviation below one microsecond across 100,000 comparisons, indicating no timing leak. Performance validation through ten-minute sustained test demonstrates 17,243 successful requests with 100% success rate and throughput of 28.74 requests/second, with latency distribution P50=3.62ms, P95=6.89ms, P99=9.13ms when connected to a single QRNG appliance. Burst testing demonstrates gateway capability exceeding 400 requests/second for short durations, with sustained throughput limited by QRNG appliance entropy generation rate. One-month continuous operation achieves zero crashes and no memory leaks. Security audit confirms correct data diode implementation with Gateway unable to initiate Collector connections even under compromise scenarios.

### 4.3 Performance Characteristics

Benchmarking with Docker containers on Windows host machine running Rust 1.75.0 release build demonstrates sustained throughput of 28.74 requests/second with median latency 3.62ms, P95 latency 6.89ms, and P99 latency 9.13ms over ten-minute sustained test achieving 100% success rate when connected to a single QRNG appliance. The gateway demonstrates burst capability exceeding 400 requests/second with sub-2-millisecond median latency when the buffer contains sufficient entropy, with sustained throughput primarily limited by the QRNG appliance's entropy generation rate of approximately 80 kilobytes per second rather than gateway processing constraints. Internal gateway processing measured via Prometheus metrics shows P50 latency of 40 microseconds and P99 latency of 114 microseconds, with HMAC verification requiring approximately 820μs and CRC32 290μs. The architecture enables horizontal scaling through multiple Gateway instances sharing collector feeds, with throughput scaling linearly with the number of QRNG appliances rather than gateway instances, as the primary bottleneck resides in quantum entropy generation capacity.
...
The five-minute replay protection window represents a security-usability tradeoff, with shorter windows providing stronger protection but operational fragility from network sensitivity, while the current configuration balances security against empirical network characteristics showing delays rarely exceeding thirty seconds. System throughput depends on the connected QRNG appliance's entropy generation rate, with typical commercial appliances generating 80-100 kilobytes per second limiting sustained request rates to approximately 29 requests per second for 1-kilobyte requests, though the gateway software itself demonstrates burst capability exceeding 400 requests per second. Organizations requiring higher sustained throughput can deploy multiple QRNG appliances with multi-source entropy aggregation. The absence of persistent entropy storage reflects current focus on real-time distribution over archival requirements.
...
QRNG-DD demonstrates that software-based data diode architecture provides practical quantum entropy distribution with performance, security, and cost characteristics suitable for academic research and moderate-security deployments, achieving sub-4-millisecond median latency and sub-10-millisecond 99th percentile latency while providing significant cost advantages over hardware solutions through MIT-licensed open-source implementation. Sustained throughput depends on the connected QRNG appliance's entropy generation capacity, typically achieving approximately 29 requests per second for 1-kilobyte requests from a single appliance, with the gateway software itself demonstrating burst capability exceeding 400 requests per second. The Model Context Protocol integration enables AI agent access to quantum randomness, eliminating integration friction through standardized tool interfaces that transform quantum randomness from specialized expertise requirement to zero-configuration commodity capability accessible through conversational interfaces.

As AI systems increasingly support scientific research and quantum computing expands, standardized access to high-quality quantum randomness emerges as critical research infrastructure. QRNG-DD provides this through open-source transparency enabling independent audits, reproducible research methodologies, and community-driven improvements, ensuring quantum entropy remains accessible worldwide rather than concentrated in well-funded institutions. The architecture scales horizontally through multiple QRNG appliances with multi-source aggregation, enabling organizations to achieve higher throughput by adding quantum entropy generation capacity rather than upgrading software infrastructure.
