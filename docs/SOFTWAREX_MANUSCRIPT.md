...
QRNG-DD is an open-source system for securely distributing quantum random numbers across network boundaries using a software-based data diode architecture. It addresses environments where QRNG appliances are confined to internal networks for security reasons, but their entropy must be safely consumed by external services, AI agents, or cloud-hosted applications. The system separates an Entropy Collector on the internal network from an Entropy Gateway on the external network; the Collector pulls entropy from QRNG hardware, signs and encapsulates it, and pushes it outward, while the Gateway exposes an authenticated REST API and Model Context Protocol (MCP) tools without any reverse communication path from external to internal networks.

Implemented in Rust, QRNG-DD exploits memory safety and asynchronous I/O to achieve low-latency, high-throughput distribution limited primarily by the QRNG hardware rather than software overhead. Integrity and replay protection are enforced by layered mechanisms including HMAC-SHA256 authentication, CRC32 checksums, timestamps, and sequence numbers. The system supports single-appliance deployments as well as multi-source entropy aggregation using XOR or HKDF-based mixing to mitigate vendor dependence and backdoor risks.

QRNG-DD integrates with AI assistants via MCP, allowing agents to request quantum-derived bytes, integers, floating-point values, UUIDs, and Monte Carlo π quality checks through standardized tools without custom client code. Built-in example applications, statistical tests, and Prometheus-based observability facilitate adoption in research and moderate-security production environments. All source code, configurations, and benchmark artifacts are provided under an MIT license to support reproducible studies and extension by the community.
...
QRNG-DD implements a three-tier architecture that separates trusted and untrusted networks through three independent components. The Entropy Collector runs on the internal network with QRNG appliance access, fetching entropy via HTTPS, buffering it in circular queues, signing packets with HMAC-SHA256, adding CRC32 checksums, and pushing them to the Gateway at configurable intervals (5 seconds by default). The Entropy Gateway runs on the external network, receives the pushed entropy, verifies HMAC and CRC32 values, buffers accepted packets in ring buffers, serves a REST API with bearer-token authentication, exports Prometheus metrics, and enforces rate limiting. A dedicated MCP server component fetches entropy from the Gateway on demand and exposes it as standardized tools for AI agents over JSON-RPC 2.0, supporting both stdio and HTTP transports.
...
Asynchronous I/O on top of a cooperative multitasking runtime allows QRNG-DD to handle many simultaneous connections with a modest thread pool, keeping network operations non-blocking and maintaining median response times under 4 milliseconds under sustained load. The Gateway demonstrates burst capability exceeding 400 requests per second when buffers are well-filled, while sustained throughput remains primarily constrained by the QRNG appliance’s entropy generation rate rather than Gateway processing capacity. When multiple QRNG sources are configured, the Collector issues requests in parallel instead of sequentially, so aggregate latency approaches the maximum of individual latencies, and source-specific timeouts prevent slow or failing appliances from degrading the performance of healthy ones.
...
The MCP server implements JSON-RPC 2.0 messaging over stdio for local AI assistants or HTTP for remote clients, exposing quantum randomness through 5 standardized tools: random bytes with configurable length (1-1048576 bytes) and encoding (hex/base64) for cryptographic keys, random integers within caller-specified ranges using modulo reduction with negligible bias for practical applications, random floats uniformly distributed in [0.0, 1.0) for Monte Carlo simulations, random UUIDs incorporating quantum entropy for unique identifiers, and randomness validation through Monte Carlo π estimation with configurable iterations (1000-100000000) returning estimates, error analysis, and 5-star quality ratings.
...
The Model Context Protocol integration enables AI agents to access quantum randomness through standardized JSON-RPC 2.0 interfaces exposing 5 tools: random bytes (1-1048576 bytes, hex/base64 encoding), random integers (configurable ranges with modulo reduction), random floats (uniform [0.0, 1.0) distribution), random UUIDs (quantum-seeded RFC 4122 v4), and randomness validation (Monte Carlo π estimation with 1000-100000000 iterations). AI assistants invoke these tools conversationally without custom integration code, receiving structured JSON responses with source attribution distinguishing quantum from pseudo-random entropy.
