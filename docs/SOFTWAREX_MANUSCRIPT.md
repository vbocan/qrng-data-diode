...
Beyond quantum randomness, the software data diode pattern generalizes to scenarios requiring unidirectional data flow including medical data extraction from clinical to research networks, industrial telemetry from operational to analytical infrastructure, and financial transaction logging from trading to compliance systems, all sharing the fundamental split architecture, push-only communication, cryptographic integrity verification, and temporal validity checking that QRNG-DD demonstrates [10].

### 1.4 Related Work and Research Gap

Hardware data diodes from vendors like Owl Cyber Defense provide physical unidirectional guarantees through fiber-optic transmission with removed receive capability [3], offering maximum security for critical infrastructure but requiring five to fifty thousand dollar investments and inflexible deployment. QRNG-DD trades physical guarantees for practical software isolation, achieving ninety-eight percent cost reduction while maintaining adequate security for research and moderate-security deployments.
...
QRNG-DD offers distinct advantages in cost-effectiveness, performance, and features compared to existing solutions. Open-source MIT licensing enables independent security audits, customization for specific requirements, and reproducible research, while zero software cost reduces five-year total solution cost by ninety-eight percent compared to hardware data diodes typically requiring fifty-five thousand dollars. Organizations with existing QRNG appliances add QRNG-DD without additional hardware investment.

Performance demonstrates substantial advantages with sustained throughput of one hundred requests per second representing twenty-fold improvement over ANU's five requests per second and median latency below ten milliseconds representing fifty-two-fold improvement over ANU's 450-millisecond average. Self-hosted deployment supporting megabyte requests contrasts with public service 1024-byte limits, while MCP integration distinguishes QRNG-DD as the only quantum service designed for AI consumption. Production observability through Prometheus metrics, structured logging, and Docker deployment provides operational capabilities absent from prototypes and public services.
...
Custom entropy source integration through trait implementation enables support for QRNG appliances beyond Quantis including Whitewood's netRandom or PicoQuant devices, with plugin registration via configuration enabling heterogeneous deployments. Custom mixing strategies address specialized requirements through strategy traits, while quality testing extends beyond Monte Carlo π to NIST SP 800-22 or Dieharder suites [12][13]. Custom MCP tools enable application-specific randomness including prime number generation, array shuffling, or distribution sampling. Integration points support webhook notifications for operational events, message queue publication for centralized logging, and custom report generation for compliance documentation.
...
The five-minute replay protection window represents a security-usability tradeoff, with shorter windows providing stronger protection but operational fragility from network sensitivity, while the current configuration balances security against empirical network characteristics showing delays rarely exceeding thirty seconds. Single-threaded push operations impose theoretical maximum twelve pushes per minute adequate for current deployments but limiting scalability for high-throughput scenarios with dozens of concurrent appliances. The absence of persistent entropy storage reflects current focus on real-time distribution over archival, though planned Entropy Archive features would record packets to tamper-evident append-only storage with cryptographic chaining.

### 6.2 Planned Enhancements

Large language model integration could reduce deployment complexity through automated network topology analysis, firewall rule generation, and complete deployment package creation including Docker Compose files and monitoring configurations, reducing deployment time from hours to minutes through natural language interaction. Enhanced quality monitoring through NIST SP 800-22 integration would provide comprehensive randomness validation with automatic degradation detection, while comparative benchmarking against pseudo-random generators would enable anomaly detection indicating hardware malfunction [12].

HTTP/2 support would improve efficiency for high-frequency small requests through multiplexing and header compression, with estimated twenty to thirty percent latency reduction benefiting interactive applications. Kubernetes operator implementation would enable cloud-native deployment with automated scaling, secret rotation, and self-healing, supporting one-command deployment to managed Kubernetes services while maintaining proper network isolation. Additional MCP transports including HTTP for remote clients and WebSocket for real-time streaming would expand AI agent accessibility beyond current stdio transport.

### 6.3 Research Directions

Historical entropy consumption data could enable investigation of correlations between quantum randomness usage and research outcomes through retrospective studies or randomized trials, providing evidence-based guidance on domains genuinely benefiting from true randomness versus high-quality pseudo-random generators. Analysis of AI-quantum interaction patterns through MCP tool invocation logging could reveal how AI systems reason about randomness, informing design of more effective AI-research tool interfaces and potentially revealing insights about AI probabilistic reasoning capabilities.

Formal verification of software data diode security properties through model checking, theorem proving, and information flow analysis would provide rigorous mathematical characterization of achievable guarantees, while quantitative comparison against hardware alternatives through adversarial capability modeling would support risk-based deployment decisions. Multi-source mixing optimization research could investigate optimal strategies for various correlation characteristics, potentially enabling machine learning techniques to automatically detect correlations and adapt mixing strategies for optimal security.

---

## 7. Conclusions

QRNG-DD demonstrates that software-based data diode architecture provides practical quantum entropy distribution with performance, security, and cost characteristics suitable for academic research and moderate-security deployments, achieving one hundred requests per second with sub-ten-millisecond latency while reducing costs by ninety-eight percent compared to hardware solutions through MIT-licensed open-source implementation. The Model Context Protocol integration represents the first quantum service designed for AI consumption, eliminating integration friction through standardized tool interfaces that transform quantum randomness from specialized expertise requirement to zero-configuration commodity capability accessible through conversational interfaces.

As AI systems increasingly support scientific research and quantum computing expands, standardized access to high-quality quantum randomness emerges as critical research infrastructure. QRNG-DD provides this through open-source transparency enabling independent audits, reproducible research methodologies, and community-driven improvements, ensuring quantum entropy remains accessible worldwide rather than concentrated in well-funded institutions. The architecture generalizes beyond quantum entropy to medical data extraction, industrial telemetry, and financial logging scenarios requiring unidirectional flow, demonstrating broader applicability through split design, push-only communication, cryptographic integrity verification, and temporal validity checking.

Future work on LLM-assisted deployment, enhanced quality monitoring, and formal security verification will strengthen research utility and production applicability. We invite contributions from quantum computing, cryptography, and AI research communities to expand capabilities, validate security properties, and apply QRNG-DD to novel use cases advancing scientific knowledge while providing practical value through democratized, accessible, and transparent quantum randomness access that accelerates discovery through reduced infrastructure barriers.
...
[2] ID Quantique, "Quantis QRNG Appliance," 2024. [Online]. Available: https://www.idquantique.com/random-number-generation/products/quantis-qrng-appliance/. [Accessed: 15-Nov-2024].

[3] Owl Cyber Defense, "Data Diode Technology," 2024. [Online]. Available: https://owlcyberdefense.com/data-diodes/. [Accessed: 15-Nov-2024].

[4] Australian National University, "ANU QRNG API," 2024. [Online]. Available: https://qrng.anu.edu.au/. [Accessed: 16-Nov-2024].

[5] National Institute of Standards and Technology, "NIST Randomness Beacon," 2024. [Online]. Available: https://beacon.nist.gov/. [Accessed: 16-Nov-2024].
...
[7] Anthropic, "Model Context Protocol Specification," 2024. [Online]. Available: https://spec.modelcontextprotocol.io/. [Accessed: 17-Nov-2024].
...
[9] Ruđer Bošković Institute, "QRNG Service," 2024. [Online]. Available: https://qrng.irb.hr/. [Accessed: 16-Nov-2024].
...
[13] R. G. Brown, "Dieharder: A Random Number Test Suite," 2024. [Online]. Available: https://webhome.phy.duke.edu/~rgb/General/dieharder.php. [Accessed: 17-Nov-2024].
